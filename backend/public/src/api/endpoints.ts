...
export const subscriptionsApi = {
  settings: () => api.get<SubscriptionSettings>("/subscriptions/settings"),
  updateSettings: (input: Record<string, unknown>) =>
    api.patch<{ ok: boolean }>("/subscriptions/settings", input),
  board: () => api.get<SubscriptionBoardRow[]>("/subscriptions/board"),
  mine: () => api.get<SubscriptionMine>("/subscriptions/mine"),
  list: (apartmentId?: number) => {
    const qs = apartmentId ? `?apartment_id=${apartmentId}` : "";
    return api.get<SubscriptionPayment[]>(`/subscriptions${qs}`);
  },
  create: (input: Record<string, unknown>) => api.post<{ id: number }>("/subscriptions", input),
  reverse: (id: number, reason: string) =>
    api.post<{ id: number }>(`/subscriptions/${id}/reverse`, { reason }),
};
